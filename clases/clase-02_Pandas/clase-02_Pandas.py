...
with open(carpeta + nombre_archivo, 'r', encoding='utf-8') as file:
    for linea in file:
...
def conseguir_listado_materias(nombre_archivo='cronograma_sugerido.csv'):
    with open(carpeta + nombre_archivo, 'rt') as file:
        lista_materias = list()
        for line in file:
...
def materias_cuatrimestre(n, nombre_archivo='cronograma_sugerido.csv'):
    lista = []
    with open(carpeta + nombre_archivo, 'rt', encoding='utf-8') as f:
        filas = csv.reader(f)
